...
exports.verifyUser = async (req, res) =>{
  try{
    ...
    if(!user){
      ...
      if(user.VT_KEY === key){
       let responce = await User.findByIdAndUpdate(userId, {isVerified : true, VT_KEY : null})
       ! responce ?  res.status(400).send({result: false, message:"error occurs", error: true}) :
...
exports.updateUserDetail = async (req, res) => {
  ...
  !user
    ? res.status(400).send({result: false, error : true , message:"invalid request" })
    : res.send({result: true, message: "Successfully password Changed!", error: null, data:{user}})
}catch(error){
  res.status(400).send({ detail: error , result: false});
}
